...
-- Session contents.
--   localoptions: window/buffer-local options, so filetype and highlighting
--                 survive a session restore (auto-session warns without it)
--   globals:      global variables, so Kulala can restore its request history
vim.opt.sessionoptions:append({ "localoptions", "globals" })
